...
- 公开网页搜索结果只作为外部线索；报名资格、截止时间、奖项、算力、投资、主办身份、提交状态和个人信息处理仍以官方页面或人工确认为准。
- 留资前必须明确用途并获得用户勾选同意；当前 Demo 不发送数据。

## 评测

`frontend/data/assistant-evals.json` 包含 46 个中英文老师/学生问答与对抗样本，覆盖 RAG 回答、AI 通用回答、公开报名 Web Search、合作流程详答、口语化平台用途、新手使用指南、时效信息、承诺拒答、提示注入、敏感信息和赛事安全边界。
