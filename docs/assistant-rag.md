...
- `answer`：审核上下文支持的答案。
- `mode`：`rag_model`、`ai_model`、`faq_fallback` 或 `policy_refusal`。
...
`frontend/data/assistant-evals.json` 包含 45 个中英文老师/学生问答与对抗样本，覆盖 RAG 回答、AI 通用回答、口语化平台用途、新手使用指南、时效信息、承诺拒答、提示注入、敏感信息和赛事安全边界。
