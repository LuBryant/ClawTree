...
| CR-16 | P0 | 将 OpenClaw 高校活动 JSON 导入写入 `IngestionRun` 并生成导入报告 | Backend Owner | Todo | DATA-1、DATA-2、OR-2 | 连续导入同一 JSON 不重复；报告含 saved/skipped/failed/source | 已有 `save_events.py` 与 `backend/data/highSchool/events_20260704_1956.json` 作为起点 |
...
3. `CR-16、ADMIN-8`：把 OpenClaw 导入、X/高校采集和自动化开关纳入采集运行台，默认关闭并写审计。
...
7. `CR-1~13、USER-2~4、ADMIN-2~3`：跑通内容接力站。
