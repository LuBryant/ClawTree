...
> AI × Web3 × Media Growth OS for multi-organization partnership campaigns.

ClawTree 是面向媒体、高校、活动方与 Web3 生态的 **AI 合作增长网络**：把分散在 X、高校官网、活动平台和公开内容源里的信号，转成可信机会、伙伴匹配、人审外联、回复漏斗与隐私安全的链上执行凭证。

**不要把大树财经（TreeFinance）理解为 ClawTree 的客户宣称或平台本身。** 当前仓库只是用大树财经高校行作为一个参考演示案例，展示同一套信号、能力库、Agent、人审和 Proof 引擎如何服务任意组织；真实客户可以通过独立 Workspace 接入自己的品牌档案、能力、来源和外联身份。
...
我们用大树财经正在推进的全球高校行、AI/Web3 活动、Space、黑客松和合作伙伴传播作为演示场景。但这也是媒体、社区、高校与生态组织普遍面对的工作流摩擦：
...
| 商业潜力 20% | 以大树财经高校行作演示案例验证闭环，再扩展到黑客松赞助、AI/Web3 活动增长和 sponsor impact report；可扩展为 campaign SaaS。 |
...
Next.js AI 客服需要从 `frontend/.env.local` 读取服务端模型配置：

    cp frontend/.env.example frontend/.env.local

填写 `DEEPSEEK_API_KEY` 后，客服采用“安全规则 + 双语 RAG + AI 多轮回答”；未填写或上游不可用时自动降级为双语 RAG FAQ，不会把密钥发送到浏览器。
